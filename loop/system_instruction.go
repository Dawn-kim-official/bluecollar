...
func buildAgentSystemInstruction(request AgentTurnRequest) string {
	...
	instruction += " You may call several tools in one response when none of them depends on another's result: they run in order and stop at the first failure. When a call's input depends on what an earlier call returns, ask for it alone and read the observation first."
